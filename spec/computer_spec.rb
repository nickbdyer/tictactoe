...
describe TicTacToe::Computer do
  ...
  it "will choose a move" do
    expect(ui).to receive(:computer_move)
    expect(computer).to receive(:chosen_move).with(board)
    computer.choose_move(board)
  end

  it "will choose the a corner if playing first" do
    expect(computer.send(:chosen_move, board)).to satisfy("be a corner") do |num|
      [0, 2, 6, 8].include? num
    end
  end
  ...
  it "will play center if the first player took a corner" do
    ...
    expect(computer.send(:chosen_move, board)).to eq 4
  end

  it "will win game if option is available" do
    board = TicTacToe::Board.new({ :grid => ["X", 2, "X", 4, 5, 6, 7, 8, 9] })
    expect(computer.send(:chosen_move, board)).to eq 1
  end
  ...
  it "will prevent edge trap" do
    ...
    expect(computer.send(:chosen_move, board)).to eq 0
  end
